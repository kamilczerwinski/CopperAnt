...
public class SimulationWithGuiSandbox extends AbstractControlSandbox {
	...
	protected void addElements(Pane root) {
		...
		computer1.getPort().setPortControl(new PortControl());
		Computer computer2 = new Computer(new IPAddress("192.168.1.2"));
		computer2.getPort().setPortControl(new PortControl());
		Cable cable = new Cable();
		cable.insertInto(computer1.getPort());
		cable.insertInto(computer2.getPort());

		simulationCanvas.addControl(computer1.getPort().getPortControl(), 0, 0);
		simulationCanvas.addControl(computer2.getPort().getPortControl(), 20, 0);
